...
trait Rule: Send + Sync {
    async fn check(&self, item: PremintTypes, context: RuleContext) -> bool;
}
...
mod test {
    ...
    async fn simple_rule(item: PremintTypes, context: RuleContext) -> bool {
        true
    }

    async fn conditional_rule(item: PremintTypes, context: RuleContext) -> bool {
        match item {
            PremintTypes::Simple(s) => s.metadata().chain_id == U256::default(),
            _ => true,
        }
    }
